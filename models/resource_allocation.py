...
def smart_allocate_hospital(
    ...
    for h in hospitals:
        dist_km = haversine_km(incident_lat, incident_lon, float(
            h["latitude"]), float(h["longitude"]))
        ...
        h_vans = [
            ...
            haversine_km(float(h["latitude"]), float(h["longitude"]), float(
                v["latitude"]), float(v["longitude"])) < 5.0
        ]
        ...
        score = (w_distance * norm_dist) + (w_beds *
                                            norm_beds) + (w_ambulances * norm_vans)
    ...
    if best_hospital is None and hospitals:
        hospitals.sort(key=lambda h: haversine_km(
            incident_lat, incident_lon, float(h["latitude"]), float(h["longitude"])))
        best_hospital = hospitals[0]
        best_distance = haversine_km(incident_lat, incident_lon, float(
            best_hospital["latitude"]), float(best_hospital["longitude"]))
